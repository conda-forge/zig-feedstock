...
if is_linux && is_cross; then
  ...
  #
  # Supersedes the old ppc64le-only `ln -sf qemu-ppc64le qemu-powerpc64le` symlink
  # into ${BUILD_PREFIX}/bin.
  _zig_qemu=""
...
if is_unix; then
  ...
  if is_cross && ! { [[ -x "${PREFIX}/lib/zig-llvm/bin/llvm-config" ]] && "${PREFIX}/lib/zig-llvm/bin/llvm-config" --version &>/dev/null; }; then
    ...
    # PR #123 round 27: with llvm-config now runnable (see the LD_LIBRARY_PATH
    # export below), CMake gets PAST Findllvm and then fails the GENERATE step
    # with CLANG_INCLUDE_DIRS / CLANG_LIBRARIES / LLD_INCLUDE_DIRS = NOTFOUND --
    # confirmed identically on linux-ppc64le (job 93301260218) and linux-riscv64
    # (job 93301260260) of run 31335756661, which also logged
    #   -- Could NOT find clang (missing: CLANG_LIBRARIES CLANG_INCLUDE_DIRS)
    #   -- Could NOT find lld (missing: LLD_LIBRARIES LLD_INCLUDE_DIRS)
    # Cause: zig's cmake/Findclang.cmake and cmake/Findlld.cmake resolve clang
    # and lld via find_path/find_library with HINTS ${LLVM_INCLUDE_DIRS} /
    # ${LLVM_LIBDIRS}, and those two come from `llvm-config --includedir` and
    # `--libdir`. Selecting the minimal, llvm-config-only BUILD_PREFIX tree above
    # therefore aims clang/lld discovery at a tree containing neither. This is
    # general to every cross lane taking this branch -- NOT ppc64le-specific (the
    # ppc64le LLD bundle override is absent on riscv64, which fails identically).
    # Findclang.cmake:12 names the supported escape hatch verbatim: "Only look
    # for Clang next to LLVM or in { CMAKE_PREFIX_PATH, CMAKE_LIBRARY_PATH,
    # CMAKE_FRAMEWORK_PATH }". None of those find_* calls pass NO_DEFAULT_PATH,
    # NO_CMAKE_PATH or NO_CMAKE_PREFIX_PATH, so CMAKE_PREFIX_PATH is honoured:
    # find_path searches <p>/include and find_library <p>/lib, matching the FULL
    # PREFIX tree's layout. PREFIX (not BUILD_PREFIX) is also the correct side
    # for conda cross builds, since it is the host prefix conda's toolchain keeps
    # inside CMAKE_FIND_ROOT_PATH.
    EXTRA_CMAKE_ARGS+=(
      -DCMAKE_PREFIX_PATH="${PREFIX}/lib/zig-llvm"
    )
  else
...
# host deps on every lane except s390x (recipe.yaml `if: not s390x`),
# reachable via zig's --search-prefix PREFIX, so drop the is_cross gate.
# riscv64 is NOT excluded any more: conda-forge now ships zlib/zstd/libxml2
