...
# Portable Python interpreter resolution (win-64 Azure may only have 'python', not 'python3')
...
# Extract exported symbol names from a COFF import-library archive by directly
# parsing ar member structure + COFF IMPORT_OBJECT_HEADER, rather than relying
# on `strings`/`nm` to tokenize the binary. Both tools have been observed to
# report 0 symbols against zig/lld's short-form PE import archives on win-64
# CI (PR#123) even when --export-all-symbols is confirmed present in the link
# command -- this is ground truth, independent of that tokenization gap.
# Reuses the same ar-parsing approach as parse_ar() above (proven correct by
# strip_atexit_from_implib's binary-surgery use of it).
#
# Usage: _extract_implib_symbols <implib_path>
#   Prints one exported symbol name per line to stdout (deduplicated).
#   Diagnostic summary goes to stderr. Returns 0 always (empty output on
#   parse failure is the caller's signal to fall back).
_extract_implib_symbols() {
  local _implib="$1"
  "${_py}" - "${_implib}" <<'PYEOF'
"""Extract exported symbol names from a COFF import library (.dll.a / .lib).

Parses the ar archive directly (same approach as strip_atexit_from_implib.sh's
parse_ar) and decodes each import member's IMPORT_OBJECT_HEADER to pull the
real symbol name, instead of relying on `strings`/`nm` to tokenize the binary.
This is ground truth: it does not depend on the member body containing
`strings`-recognizable printable runs or on `nm`/`llvm-nm` understanding the
short-form PE import-archive format.

COFF short import format (winnt.h IMPORT_OBJECT_HEADER, 20 bytes):
  Sig1            u16  IMAGE_FILE_MACHINE_UNKNOWN (0x0000)
  Sig2            u16  IMPORT_OBJECT_HDR_SIG2 (0xFFFF)
  Version         u16
  Machine         u16
  TimeDateStamp   u32
  SizeOfData      u32  (length of the two following NUL-terminated strings)
  OrdinalOrHint   u16
  Flags           u16  (bits 0-1 = import type, bits 2-4 = name type)
followed by: symbol name (NUL-terminated), then DLL name (NUL-terminated).

A member that does NOT start with Sig1=0x0000 Sig2=0xFFFF is a regular (long
form) COFF object, not a short import -- fall back to a best-effort scan of
its COFF symbol table for external names.
"""
import sys
import struct
...
def short_import_symbol(body):
    """Decode IMPORT_OBJECT_HEADER; return (symbol_name, dll_name) or None."""
    if len(body) < 20:
        return None
    ...
    dll = rest[:nul2] if nul2 >= 0 else rest
    try:
        return (sym.decode('ascii'), dll.decode('ascii', errors='replace'))
    except UnicodeDecodeError:
        return None


def long_import_symbols(body):
    """Best-effort: pull external symbol names from a regular COFF object's
    symbol table (used only if a member is NOT a short-form import)."""
    if len(body) < 20:
        return []
    try:
        _machine, _nsects, _ts, symtab_off, nsyms = struct.unpack('<HHIII', body[0:16])
    except struct.error:
        return []
    if symtab_off == 0 or nsyms == 0 or symtab_off + nsyms * 18 > len(body):
        return []
    strtab_off = symtab_off + nsyms * 18
    strtab = body[strtab_off:] if strtab_off < len(body) else b''
    names = []
    i = 0
    while i < nsyms:
        rec = body[symtab_off + i * 18: symtab_off + i * 18 + 18]
        if len(rec) < 18:
            break
        zeroes, offset = struct.unpack('<II', rec[0:8])
        naux = rec[17]
        if zeroes == 0:
            nul = strtab.find(b'\x00', offset - 4)
            name = strtab[offset - 4:nul if nul >= 0 else len(strtab)]
        else:
            name = rec[0:8].rstrip(b'\x00')
        try:
...
def main():
    ...
    seen = set()
    count = 0
    for _off, hdr, body in members:
        nm = member_name(hdr).rstrip()
        if nm in ('/', '//'):
            continue
        short = short_import_symbol(body)
        if short is not None:
            ...
            if sym and sym not in seen:
                ...
                count += 1
            continue
        for sym in long_import_symbols(body):
            if sym and sym not in seen:
                seen.add(sym)
                print(sym)
                count += 1
    print(f"    ar-parse: {len(members)} member(s), {count} distinct exported symbol(s)",
          file=sys.stderr)
