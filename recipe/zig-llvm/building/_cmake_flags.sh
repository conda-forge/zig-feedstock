...
#
# riscv64 is excluded from this pin: it does not use the zig-zstd vendored
# layout (no vendoring for riscv64 -- conda-forge ships zstd for it), and is
# pinned to the plain $PREFIX layout in the riscv64 block below instead.
# --------------------------------------------------------------------
...
# riscv64: conda-forge now ships zstd for linux-riscv64, so $PREFIX carries a
# target-arch libzstd and the old "disable zstd entirely" workaround is obsolete.
# The original failure was NOT path ordering -- _llvm_build.sh:201 already puts
# $PREFIX ahead of $BUILD_PREFIX -- it was that $PREFIX had no zstd at all, so
# find_package fell through to $BUILD_PREFIX's native x86_64 libzstd.so and the
# target link failed with "libzstd.so is incompatible with elf64lriscv".
# Pin explicitly anyway: typed (:PATH/:FILEPATH) so CMake stores them
# INITIALIZED, per the note above -- untyped vars are ignored by find_package.
if [[ "${target_platform}" == "linux-riscv64" ]] && [[ -f "${PREFIX}/lib/libzstd.so" ]]; then
    CMAKE_PLATFORM_FLAGS+=(
        -DLLVM_ENABLE_ZSTD=ON
        -Dzstd_ROOT:PATH="${PREFIX}"
        -Dzstd_LIBRARY:FILEPATH="${PREFIX}/lib/libzstd.so"
        -Dzstd_INCLUDE_DIR:PATH="${PREFIX}/include"
    )
    dbg "cmake_flags: riscv64 zstd pinned to conda-forge \$PREFIX layout"
elif [[ "${target_platform}" == "linux-riscv64" ]]; then
    dbg "cmake_flags: WARNING riscv64 but no ${PREFIX}/lib/libzstd.so - zstd left to CMAKE_PREFIX_PATH"
fi
if is_osx; then
