function build_lld_bundle() {
  ...
  # LLVM major differs by lane (fresh-built LLVM on native vs older build-dep LLVM on cross) and
  # CMake may emit dashed libLLVM-NN.so or dotted libLLVM.so.NN.N (see post-install.sh) / libLLVM-NN.dll.a.
  # Resolve whatever libLLVM is actually present instead of hardcoding a version.
  local _llvm_shared _llvm_implib
  _llvm_shared=$(ls "${_lld_lib}"/libLLVM-*.so "${_lld_lib}"/libLLVM.so.* 2>/dev/null | head -n1)
  _llvm_implib=$(ls "${_lld_lib}"/libLLVM-*.dll.a 2>/dev/null | head -n1)
  ...
  if is_linux; then
    ...
    # PR #123 osx fix: the bundle deliberately does NOT link its own -lLLVM.
    # zig2 links libLLVM and calls InitializeAllTargets to populate LLVM's
    # per-image TargetRegistry singleton; when zig2 loads liblldZig.dylib,
    # lld::macho::link must see that SAME registry. A bundle-owned -lLLVM can
    # resolve to a SECOND libLLVM image with an empty registry, so
    # `zig cc -fuse-ld=lld` fails 'unable to create target: no targets are
    # registered' (osx-arm64). -Wl,-undefined,dynamic_lookup leaves LLVM symbols
    # unresolved at build time and binds them to the host process's already-loaded
    # libLLVM at load time -- single image, single registry.
