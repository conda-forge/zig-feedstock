#!/usr/bin/env bash
# Force-load wrapper for zig cc on macOS.
#
...
# passes them directly to zig cc.
#
# Usage: zig-force-load-cc <all normal zig cc args>
...
if [[ ${_all_load} -eq 1 ]]; then
    ...
    for _a in "${_other_args[@]}"; do
        if [[ "$_a" == *.a ]] && [[ -f "$_a" ]]; then
            # Resolve to absolute — ar x runs after cd to a different directory
...
for _a in "${_force_load_archives[@]}"; do
    if [[ -f "$_a" ]]; then
        ...
        echo "WARNING: zig-force-load-cc: archive not found: $_a" >&2
    fi
done
