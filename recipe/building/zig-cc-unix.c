 ...
 *
 * Replaced recipe/scripts/{zig-cc,zig-cxx,zig-ar,zig-ranlib,zig-rc,zig-lld,
 * zig-windres,zig-asm,zig-force-load-cc,zig-force-load-cxx}.sh, all now
 * removed.
 *
 * run_cc() is a port of the former recipe/scripts/_zig-cc-common.sh
 * (sourced by zig-cc.sh / zig-cxx.sh; both since removed).  The R1-R13
 ...
 * then anything -- this is a shell glob, not a literal string;
 * ported the same way _translate.inc:63-75 ports the sibling
 * -Wl,-O* case for R9. */
...
/*
 * Raw single-token LLD-trigger scan, EXCLUDING the "-Xlinker) ..." arm
 * itself (handled separately by the -Xlinker lookahead below) and
 * EXCLUDING "-fuse-ld=lld" (checked by the caller directly, folded in
 * here for a single call site).
 */
static int is_lld_trigger(const char *arg) {
    if (str_eq(arg, "-fuse-ld=lld")) return 1;
    if (str_eq(arg, "-Wl,--version-script") ||
        starts_with(arg, "-Wl,--version-script,")) return 1;
    if (str_eq(arg, "-Wl,--dynamic-list") ||
        starts_with(arg, "-Wl,--dynamic-list,") ||
        starts_with(arg, "-Wl,--dynamic-list=")) return 1;
    if (str_eq(arg, "-Wl,-z,defs") || str_eq(arg, "-Wl,-z,nodelete")) return 1;
    if (str_eq(arg, "-Wl,--gc-sections") || str_eq(arg, "-Wl,--no-gc-sections")) return 1;
    if (str_eq(arg, "-Wl,--build-id") || starts_with(arg, "-Wl,--build-id=")) return 1;
    if (str_eq(arg, "-Wl,--allow-shlib-undefined") ||
        str_eq(arg, "-Wl,--no-allow-shlib-undefined")) return 1;
    ...
    if (is_wl_o_digit(arg)) return 1;
    if (str_eq(arg, "-Wl,-exported_symbols_list") ||
        starts_with(arg, "-Wl,-exported_symbols_list,")) return 1;
    if (str_eq(arg, "-Wl,-unexported_symbols_list") ||
        starts_with(arg, "-Wl,-unexported_symbols_list,")) return 1;
    if (str_eq(arg, "-Wl,-reexported_symbols_list") ||
        starts_with(arg, "-Wl,-reexported_symbols_list,")) return 1;
    if (str_eq(arg, "-Wl,-force_symbols_not_weak_list") ||
        starts_with(arg, "-Wl,-force_symbols_not_weak_list,")) return 1;
    if (str_eq(arg, "-Wl,-force_symbols_weak_list") ||
        starts_with(arg, "-Wl,-force_symbols_weak_list,")) return 1;
    if (str_eq(arg, "-Wl,-all_load") || starts_with(arg, "-Wl,-force_load,")) return 1;
 ...
 * literal "-Xlinker". Note this list is NOT identical to
 * is_lld_trigger(): e.g. bare "--gc-sections" only triggers here (as an
 * -Xlinker argument), never as a standalone token.
 */
static int is_xlinker_lld_trigger(const char *arg) {
    if (str_eq(arg, "--dynamic-list") || starts_with(arg, "--dynamic-list=") ||
        str_eq(arg, "--version-script") || starts_with(arg, "--version-script=")) return 1;
    if (str_eq(arg, "--gc-sections") || str_eq(arg, "--no-gc-sections") ||
        str_eq(arg, "--build-id") || starts_with(arg, "--build-id=")) return 1;
    if (str_eq(arg, "--allow-shlib-undefined") ||
        str_eq(arg, "--no-allow-shlib-undefined")) return 1;
    if (str_eq(arg, "-exported_symbols_list") ||
        starts_with(arg, "-exported_symbols_list,")) return 1;
    if (str_eq(arg, "-unexported_symbols_list") ||
        starts_with(arg, "-unexported_symbols_list,")) return 1;
    if (str_eq(arg, "-all_load") || str_eq(arg, "-force_load") ||
        starts_with(arg, "-force_load,")) return 1;
    return 0;
}

/* -Xlinker <arg> pairs to drop entirely (both tokens). */
 ...
 * runtime libs zig doesn't ship / can't link); everything not matched
 * here is kept.
 */
static int is_post_translate_drop(const char *arg) {
    return starts_with(arg, "-march=") || starts_with(arg, "-mtune=") ||
           ...
           str_eq(arg, "-fstack-protector") || str_eq(arg, "-fno-plt") ||
           str_eq(arg, "-fno-partial-inlining") ||
           str_eq(arg, "-fno-ipa-cp-clone") ||
           starts_with(arg, "-fdebug-prefix-map=") ||
           starts_with(arg, "-stdlib=") ||
           str_eq(arg, "-lgcc_eh") || str_eq(arg, "-lgcc_s") ||
           str_eq(arg, "-l:libpthread.a") || is_l_libpthread_so(arg);
}
...
static int run_cc(const char *zig_bin, const char *prog, int mode_is_cxx,
     ...
     * 6-element _sysroot_flags group. */
    ...
    /* ---- STEP 2+3: raw LLD-trigger scan fused with the -Xlinker
     * general pre-filter. Both are independent passes over the SAME
     * raw argv in bash (two separate loops that don't share state),
     * so fusing them into one loop is behaviorally equivalent and
     * mirrors zig-cc-nonunix.c's style. bash's `break` on first
     * LLD-trigger match is dropped (harmless: OR-ing a boolean flag
     ...
     * against the lookahead-only list, so a token that is itself
     * being CONSUMED as an -Xlinker value is never reprocessed as a
     * fresh "-Xlinker" trigger point even if it happens to equal the
     * literal string "-Xlinker" (e.g. pathological input "-Xlinker
     * -Xlinker foo"). This ordering is a deliberate deviation from
     * zig-cc-nonunix.c for exactness against the bash source. */
    int raw_argc = argc - 1;
    ...
    int use_lld_raw = 0;
    {
         ...
         * (the `if` guarding the next-token lookup fails). */
    }

    /* ---- STEP 4: fill the translate profile and delegate the
     * R1-R9 de-dup rules to the generated translator. ---- */
    ...
    /* mode_is_cxx is the caller's parameter (caller-owns-init per
    ...
    /* ---- STEP 8: final mode + macOS deployment-target rewrite of
     * the target triple. ---- */
    ...
    if (macos_dep && *macos_dep && strstr(ZIG_TARGET, "-macos") != NULL) {
        /* ${_zig_target%%-macos*} removes the LONGEST
    ...
    /* ---- STEP 9: inject -fuse-ld=lld only if promoted and not
     * already present. ---- */
    int has_lld_flag = 0;
    for (i = 0; i < fi; i++) {
    ...
    /* ---- STEP 10: inject a default -target only if the filtered
     * args contain neither -target nor any --target= prefixed
     * token. ---- */
    int has_target = 0;
    for (i = 0; i < fi; i++) {
    ...
    /* ---- STEP 11: assemble the final argv and exec. argv[0] is the
     * zig binary path itself, matching zig-cc.sh/zig-cxx.sh's `exec
     * "@ZIG_BIN@" "${_exec_args[@]}"` (bash sets argv[0] to the
     * exec'd command name, here @ZIG_BIN@). Order: argv0, mode,
     * [lld], [target x2], sysroot flags, filtered args, NULL --
     * matching bash's
...
/* Item 14: trivial exec passthrough.
...
/* Item 10: strip the 'T' (thin archive) modifier.
...
/* Item 11: ld.lld for ELF, ld64.lld for Mach-O.
...
/* Item 12: rewrite -o <out> to -fo <out> and -o<out>
...
/* Note this is inventory item 2's DUPLICATED sysroot block:
