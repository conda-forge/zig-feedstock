...
echo "DBG zig_build cmake: CC=${ZIG_CC:-unset} CXX=${ZIG_CXX:-unset}"
echo "DBG zig_build llvm: ZIG_LLVM_ROOT=${ZIG_LLVM_ROOT} LLVM_CONFIG_EXE=${LLVM_CONFIG_EXE}"
...
# Linux: append libc++.so.1 so the final zig link uses zig's libc++ (std::__1
# namespace) matching libzigcpp.a, rather than libstdc++ (std:: namespace).
# Mirrors the macOS pattern above.  ZIG_SHARED_LIBCXX_DIR=${PREFIX}/lib/zig-llvm/lib
# (set unconditionally at line 192).
