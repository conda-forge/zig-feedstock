...
Installing `zig` from the `conda-forge/label/zig_rc` channel can be achieved by adding `conda-forge/label/zig_rc` to your channels with:

```
conda config --add channels conda-forge/label/zig_rc
conda config --set channel_priority strict
```

Once the `conda-forge/label/zig_rc` channel has been enabled, `zig, zig-compiler, zig_impl_linux-64, zig_impl_linux-aarch64, zig_impl_linux-ppc64le, zig_impl_linux-riscv64, zig_impl_osx-64, zig_impl_osx-arm64, zig_impl_win-64, zig_linux-64, zig_linux-aarch64, zig_linux-ppc64le, zig_linux-riscv64, zig_linux-s390x, zig_osx-64, zig_osx-arm64, zig_win-32, zig_win-64, zig_win-arm64` can be installed with `conda`:
...
```
conda search zig --channel conda-forge/label/zig_rc
```

or with `mamba`:

```
mamba search zig --channel conda-forge/label/zig_rc
```
...
mamba repoquery search zig --channel conda-forge/label/zig_rc

# List packages depending on `zig`:
mamba repoquery whoneeds zig --channel conda-forge/label/zig_rc

# List dependencies of `zig`:
mamba repoquery depends zig --channel conda-forge/label/zig_rc
```
